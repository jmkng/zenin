use crate::engine::{Engine, MetricKind, MetricUnit, SeriesId};
use crate::medic::{
    MedicCpuStat,
    medic_cpu_agg_stat,
...
fn reg_cpu_time(engine: &mut Engine, mode_name: &str) -> SeriesId {
    engine.register(
        "cpu.time",
        &[("mode", mode_name)],
        MetricUnit::Count,
...
impl MedicCpuProbe {
    pub fn new(engine: &mut Engine) -> Self {
        let total = CoreHandles {
            user: reg_cpu_time(engine, "user"),
            system: reg_cpu_time(engine, "system"),
            nice: reg_cpu_time(engine, "nice"),
            idle: reg_cpu_time(engine, "idle"),
        };
    ...
    pub fn emit(&mut self, mut emit: impl FnMut(SeriesId, f64)) {
        unsafe {
